...
-- About
-- The seller of the ADA creates a SWAP contract. So the smart contract contains ADA and Oracle to be used for exchange.
-- Then the smart contract can be used in one of two ways. Either a swap doesn't occur and the seller requests the ADA back or a swap does occur
...
mkSwapValidator oracle addr pkh () ctx =    -- takes two script inputs and another input: 1) Oracle for exchange rate 2) Swap outputs with lovelaces; other input is the funds from buyer
    ...
    txSignedBy info pkh ||                  -- This is the case where the swap doesn't occur and seller signs the swap contract to get back the Ada that they intended to swap 
